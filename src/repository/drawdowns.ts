...
function parseDrawdowns(prices: Price[]): Drawdown[] {
...
export async function getDrawdowns(
  stockName: string
): Promise<Drawdown[] | null> {
  if (cacheEmpty) {
  ...
  if (!(stockName && cache.has(stockName))) {
    return null;
  }

  const prices = cache.get(stockName);
  return parseDrawdowns(prices as Price[]);
}
